...
from wakil.app.ingest_service import (
    ...
    _has_dominant_recurring_subject,
    _is_noise_candidate,
    _is_unpopulated_stub,
    _looks_like_personal_reflection,
    _merge_entity_note,
...
def _capture_text(workspace, kb_path, content: str, *, filename="clipping.md", context=None) -> int:
    """Capture a source_type='text' source with the given raw content —
    sibling to _capture above, which is transcript-only."""
    path = kb_path / filename
    path.write_text(content)
    proposal = prepare_capture(workspace, "text", _capture_client(), file=path, context=context)
    return apply_capture(workspace, proposal).source_id
...
#
# The reported bug: a hand-authored, multi-date personal reflection
# (discussing 1-2 external works in passing), captured as source_type
# "text", never lands as a journal entry -- it splits into empty stub
# entities for the referenced works instead. Root cause: proposed_note's
# type is decided entirely by extraction (_run_extraction / skills/text/
# SKILL.md); prior guidance attempts (#75, PR #83) edited entity-resolve/
# SKILL.md instead, a different model call that can never affect this
# outcome. These fixtures double as the heuristic's own test corpus: one
# true positive (the reported shape) and four non-reflective counter-shapes
# that each share at least one individual signal with it.

_TP_REFLECTION_TEXT = """\
# Weekly reflection

## 2026-07-20
Today I spent time thinking about my career direction. I read a chapter of
Atomic Habits and found myself questioning some of its claims about identity
change. My main takeaway was that small wins matter more than I expected.

## 2026-07-24
I met an old friend for coffee this afternoon. We talked about my plans for
the next quarter and how uncertain I feel about the path ahead. I also
skimmed through Deep Work again on the train home.

## 2026-07-26
I have been reflecting on my goals all week. I feel like I need to
reprioritize my time and be more honest with myself about what matters.
"""

_BOOK_NOTE_TEXT = """\
# Deep Work by Cal Newport

Deep Work argues that the ability to focus without distraction is
increasingly rare and increasingly valuable. Newport distinguishes deep work
from shallow work and lays out rules for cultivating a deep work habit,
including time blocking and embracing boredom.

Newport's core claim is that deep work produces outsized results relative to
shallow, reactive work, and that most knowledge workers have let shallow work
crowd out their capacity for depth.
"""

_MEETING_TRANSCRIPT_TEXT = """\
# Kickoff call

## 2026-07-09
Jane: let's prototype FNOL routing with graph memory.
Bob: agreed, I can start on the schema this week.
Jane: I'll set up the review meeting.

## 2026-07-16
Bob: I finished the schema draft, sharing it with Jane now.
Jane: I reviewed it, looks solid. Let's move to implementation.
"""

_REFERENCE_ARTICLE_TEXT = """\
# How graph databases handle traversal

Graph databases represent data as nodes and edges rather than tables. This
lets them execute multi-hop traversal queries far more efficiently than a
relational join chain. Neo4j, for instance, uses index-free adjacency to
keep traversal cost proportional to the size of the matched pattern rather
than the size of the overall dataset.
"""
...
## 2026-06-01
Started framing the deck today. I cut the joists to length and set the
first posts for the deck frame. My back is going to hurt tomorrow.

## 2026-06-08
I finished attaching the ledger board for the deck and started laying
decking boards. Slow going but the deck is looking good so far.

## 2026-06-15
I sealed the deck today and cleaned up the site. Pretty happy with how the
...
@pytest.mark.parametrize(
    "text",
    [
        pytest.param(_BOOK_NOTE_TEXT, id="book-note"),
        pytest.param(_MEETING_TRANSCRIPT_TEXT, id="meeting-transcript"),
        pytest.param(_REFERENCE_ARTICLE_TEXT, id="reference-article"),
        pytest.param(_BUILD_LOG_TEXT, id="build-log"),
    ],
)
def test_looks_like_personal_reflection_does_not_misfire_on_non_reflective_shapes(text):
    assert _looks_like_personal_reflection(text) is False


def test_dominant_recurring_subject_is_what_excludes_the_build_log():
    # Confirms *why* the build log above is excluded from the heuristic --
    # its recurring project noun ("deck") appears in every dated section,
    # unlike the reflection's scattered, one-off external references. Guards
    # the actual discriminating signal directly, not just the end-to-end
    # outcome above.
    assert _has_dominant_recurring_subject(_BUILD_LOG_TEXT) is True
    assert _has_dominant_recurring_subject(_TP_REFLECTION_TEXT) is False


def test_prepare_enrichment_hints_extraction_prompt_for_reflection_shaped_text(workspace, kb_path):
    source_id = _capture_text(workspace, kb_path, _TP_REFLECTION_TEXT)
    payload = dict(MODEL_JSON, proposed_note=None)  # extraction "misses" the shape
    client = FakeClient([payload, RESOLUTION_JSON, REVISION_JSON, STUB_SYNTHESIS_JSON])

    prepare_enrichment(workspace, source_id, client)

    extraction_prompt = client.calls[0][1]
    assert "mechanically resembles a personal reflection" in extraction_prompt


def test_prepare_enrichment_warns_when_reflection_shape_not_journaled(workspace, kb_path):
    source_id = _capture_text(workspace, kb_path, _TP_REFLECTION_TEXT)
    payload = dict(MODEL_JSON, proposed_note=None)
    client = FakeClient([payload, RESOLUTION_JSON, REVISION_JSON, STUB_SYNTHESIS_JSON])

    proposal = prepare_enrichment(workspace, source_id, client)

    assert any(
        "mechanically resembles a personal reflection" in warning
        and "proposed no note at all" in warning
        for warning in proposal.warnings
    )


def test_prepare_enrichment_does_not_warn_when_reflection_shape_correctly_journaled(
    workspace, kb_path
):
    source_id = _capture_text(workspace, kb_path, _TP_REFLECTION_TEXT)
    payload = dict(
        MODEL_JSON,
        proposed_note={
            "path": "journal/2026/2026-07-26-weekly-reflection.md",
            "markdown": (
                "---\ntype: journal\ndate: 2026-07-26\nweek: '2026-W30'\nday: sun\n"
                "year: 2026\ntitle: Weekly reflection\ntopics: [career, reading]\n"
                "tags: [reflection]\ncreated: 2026-07-26\n---\n\n"
                "# Weekly reflection\n\nA week of thinking about career direction.\n"
            ),
        },
    )
    client = FakeClient([payload, RESOLUTION_JSON, REVISION_JSON, STUB_SYNTHESIS_JSON])
    ...
    assert not any(
        "mechanically resembles a personal reflection" in warning for warning in proposal.warnings
    )


def test_prepare_enrichment_does_not_hint_for_non_text_source_types(workspace, transcript):
    # Scoped to source_type "text" (see prepare_enrichment) -- a captured
    # meeting transcript never gets the reflection heuristic applied at all,
    ...
    extraction_prompt = client.calls[0][1]
    assert "mechanically resembles a personal reflection" not in extraction_prompt

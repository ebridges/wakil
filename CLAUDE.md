...
- **`docs/DEVELOPMENT.md`** — recurring dev patterns worth generalizing
  (mandatory reading; see Working Agreement item 14 below).
- **`docs/TROUBLESHOOTING.md`** — a dated, append-only log of real bugs and
  gotchas that cost debugging time (mandatory reading; see item 14 below).
- **`docs/pr-review-policy.md`** — how to triage `pr-reviewer` findings
  (fix / file an issue / decline). Read it before acting on a review; the
  default is *not* to fix everything.
...
13. When acting on `pr-reviewer` feedback, triage it against
    ...
    code is what produced the regression that policy's own example records.
14. **Mandatory every run that does substantive work (not a trivial
